...
class aerodynamics:
    ...
    def update_sattelite_state(self):
        ...
        try:
            # satellite will not have a state defines in very first propagation step
        ...
        if not self.first_iteration:
            ...
            if self.print_sim_progress:
                if time.time() - self.last_t_print_sim_progress > 5:
                    # more than 5 seconds have passed since last progress report, print a new one
    ...
    def get_body_fixed_angular_velocity(self):
        ...
        if not self.first_iteration:
            ...
            print(body_fixed_angular_velocity)
...
class Orbit:
    ...
    def simulate(self,
        ...
        initial_state_rotational = np.hstack(
            (initial_state_rotational, [np.deg2rad(0), np.deg2rad(0), np.deg2rad(100)]))
...
def analyse_simple_sim():
    ...
    plt.scatter(time_hours, data[-1], s=30, alpha=0.3, edgecolors='k')
